...
public class CampfireCookingScreen extends ContainerScreen<CampfireCookingContainer> implements IContainerListener {
    ...
    private final CampfireCookingContainer campfireCookingContainer;
    JSONObject jsonPacket = new JSONObject();

    //gui本地化
    TranslationTextComponent RECIPE_NAME = new TranslationTextComponent("gui."+DPRM.MOD_ID+".campfire_cooking.recipe_name");//配方名
    TranslationTextComponent GROUP_NAME = new TranslationTextComponent("gui."+DPRM.MOD_ID+".campfire_cooking.group_name");//组名
    TranslationTextComponent ADD_RECIPE = new TranslationTextComponent("gui."+DPRM.MOD_ID+".campfire_cooking.add_recipe");//添加配方
    TranslationTextComponent ADD_TO_DATAPACK = new TranslationTextComponent("gui."+DPRM.MOD_ID+".campfire_cooking.add_to_datapack");//添加至数据包
    TranslationTextComponent PLEASE_COMPLETE_RECIPE_INFO = new TranslationTextComponent("gui."+DPRM.MOD_ID+".campfire_cooking.please_complete_recipe_info");//完善配方信息
    ...
    public CampfireCookingScreen(CampfireCookingContainer campfireCookingContainer, PlayerInventory inv, ITextComponent titleIn) {
        ...
        this.campfireCookingContainer = campfireCookingContainer;
        this.jsonPacket = JSON.parseObject(campfireCookingContainer.getPacketBuffier().readString());
    }
    @Override
    protected void init() {
        super.init();
        ...
        this.confirmBtn = new Button(this.guiLeft - 85, this.guiTop + 85, 80, 20, ADD_RECIPE.getString(), (button) -> {
            ...
            JSONObject campfireCookingRecipe = JsonManager.genCampfireCookingRecipe(campfireCookingContainer.furnaceSlots, groupNameInput.getText());
            ...
            if (result.getBoolean("success")){
                ...
                playerInventory.player.sendMessage(new TranslationTextComponent("gui."+DPRM.MOD_ID+".campfire_cooking.recipe_generate_successed",result.getString("dir")));
            }else {
                playerInventory.player.sendMessage(new TranslationTextComponent("gui."+DPRM.MOD_ID+".campfire_cooking.recipe_generate_failed",result.getString("dir")));
            }
    ...
    public void tick() {
        super.tick();
        Slot[] slots = campfireCookingContainer.furnaceSlots;
