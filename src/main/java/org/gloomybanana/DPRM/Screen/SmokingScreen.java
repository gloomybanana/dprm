...
public class SmokingScreen extends ContainerScreen<SmokingContainer> implements IContainerListener {
    ...
    private final SmokingContainer smokingContainer;
    JSONObject jsonPacket = new JSONObject();

    //gui本地化
    TranslationTextComponent RECIPE_NAME = new TranslationTextComponent("gui."+DPRM.MOD_ID+".smoking.recipe_name");//配方名
    TranslationTextComponent GROUP_NAME = new TranslationTextComponent("gui."+DPRM.MOD_ID+".smoking.group_name");//组名
    TranslationTextComponent ADD_RECIPE = new TranslationTextComponent("gui."+DPRM.MOD_ID+".smoking.add_recipe");//添加配方
    TranslationTextComponent ADD_TO_DATAPACK = new TranslationTextComponent("gui."+DPRM.MOD_ID+".smoking.add_to_datapack");//添加至数据包
    TranslationTextComponent PLEASE_COMPLETE_RECIPE_INFO = new TranslationTextComponent("gui."+DPRM.MOD_ID+".smoking.please_complete_recipe_info");//完善配方信息
    ...
    public SmokingScreen(SmokingContainer smokingContainer, PlayerInventory inv, ITextComponent titleIn) {
        ...
        this.smokingContainer = smokingContainer;
        this.jsonPacket = JSON.parseObject(smokingContainer.getPacketBuffier().readString());
    }
    ...
    protected void init() {
        ...
        this.confirmBtn = new Button(this.guiLeft - 85, this.guiTop + 85, 80, 20, ADD_RECIPE.getString(), (button) -> {
            ...
            JSONObject smokingRecipe = JsonManager.genSmokingRecipe(smokingContainer.furnaceSlots, groupNameInput.getText());
    ...
    public void tick() {
        super.tick();
        Slot[] slots = smokingContainer.furnaceSlots;
