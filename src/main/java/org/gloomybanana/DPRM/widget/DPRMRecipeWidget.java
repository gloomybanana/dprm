...
public class DPRMRecipeWidget extends Widget {
    ...
    public DPRMRecipeWidget(int guiLeft, int guiTop, int recipeIndex, JSONObject recipe, JSONObject jsonPacket) {
        super(guiLeft + 11 + 25 * (recipeIndex % 5), guiTop + 31 + 25 * (recipeIndex / 5), 24, 24, new TranslationTextComponent(""));
        this.recipe = recipe;
        this.jsonPacket = jsonPacket;
    }

    @Override
    public void renderButton(MatrixStack matrixStack,int mouseX, int mouseY, float partTick) {
        ...
        if (result_count>1){
            Minecraft.getInstance().getItemRenderer().renderItemOverlayIntoGUI(this.font,resultItemStack,this.x+4,this.y+4,result_count.toString());
        }
    }
