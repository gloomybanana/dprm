...
public class SmeltingScreen extends ContainerScreen<SmeltingContainer> implements IContainerListener {
    private final ResourceLocation SMELTING_TEXTURE = new ResourceLocation(DPRM.MOD_ID, "textures/gui/smelting.png");
    private final SmeltingContainer smeltingContainer;
    JSONObject jsonPacket = new JSONObject();

    //gui本地化
    TranslationTextComponent RECIPE_NAME = new TranslationTextComponent("gui."+DPRM.MOD_ID+".smelting.recipe_name");//配方名
    TranslationTextComponent GROUP_NAME = new TranslationTextComponent("gui."+DPRM.MOD_ID+".smelting.group_name");//组名
    TranslationTextComponent ADD_RECIPE = new TranslationTextComponent("gui."+DPRM.MOD_ID+".smelting.add_recipe");//添加配方
    TranslationTextComponent ADD_TO_DATAPACK = new TranslationTextComponent("gui."+DPRM.MOD_ID+".smelting.add_to_datapack");//添加至数据包
    TranslationTextComponent PLEASE_COMPLETE_RECIPE_INFO = new TranslationTextComponent("gui."+DPRM.MOD_ID+".smelting.please_complete_recipe_info");//完善配方信息
    ...
    public SmeltingScreen(SmeltingContainer smeltingContainer, PlayerInventory inv, ITextComponent titleIn) {
        super(smeltingContainer, inv, titleIn);
        this.smeltingContainer = smeltingContainer;
        this.jsonPacket = JSON.parseObject(smeltingContainer.getPacketBuffier().readString());
    }
    ...
    protected void init() {
        ...
        this.confirmBtn = new Button(this.guiLeft - 85, this.guiTop + 85, 80, 20, ADD_RECIPE.getString(), (button) -> {
            ...
            JSONObject smeltingRecipe = JsonManager.genSmeltingRecipe(smeltingContainer.furnaceSlots, groupNameInput.getText());
            JSONObject result = JsonManager.createJsonFile(jsonPacket,smeltingRecipe,recipeNameInput.getText());
            if (result.getBoolean("success")){
                System.out.println(result.getString("dir"));
                playerInventory.player.sendMessage(new TranslationTextComponent("gui."+DPRM.MOD_ID+".smelting.recipe_generate_successed",result.getString("dir")));
            }else {
                playerInventory.player.sendMessage(new TranslationTextComponent("gui."+DPRM.MOD_ID+".smelting.recipe_generate_failed",result.getString("dir")));
            }
    ...
    public void tick() {
        super.tick();
        Slot[] slots = smeltingContainer.furnaceSlots;
    ...
    protected void drawGuiContainerBackgroundLayer(float partialTicks, int mouseX, int mouseY) {
        ...
        this.minecraft.getTextureManager().bindTexture(SMELTING_TEXTURE);
        ...
        //熔炉图标
        this.minecraft.getItemRenderer().renderItemAndEffectIntoGUI(new ItemStack(Items.FURNACE),guiLeft+56,guiTop+54);
    }
    @Override
